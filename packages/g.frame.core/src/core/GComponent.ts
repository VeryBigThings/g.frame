...
export class GComponent extends EventDispatcher<string> {
    ...
    constructor() {
        ...
        this.uiObject.userData.GComponent = this;

        this.disposeObjects = [];
    }

    /**
     * Function to get world position of this GComponent
     ...
     * @param {Object3D} parentObject=this.uiObject Where to add object. By default it is uiObject of the GComponent
     * @return {Mesh|GComponent} object
     */
     ...
     * @param disposeParams
     */
    disposeObject(object?: Object3D | GComponent, disposeParams?: any) {
     ...
     */
    pipe(pipe: Pipe<GComponent>) {
        const that = pipe(this);
        if (that !== this) {
            console.error(
                'Next pipe changed returned new instance of GComponent (pipe, this, that)',
                pipe,
                this,
                that
            );
