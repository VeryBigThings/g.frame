...
export class OrbitControls extends EventDispatcher {
    ...
    dollyEnd: Vector2;
    dollyDelta: Vector2;
    updateLastPosition: Vector3;
    updateOffset: Vector3;
    updateQuat: Quaternion;
    updateLastQuaternion: Quaternion;
    updateQuatInverse: Quaternion;
    panLeftV: Vector3;
    panUpV: Vector3;
    panInternalOffset: Vector3;
    ...
    /**
     * get controls object (camera) position.
     * If isVR get camera parent position
     */
    ...
    public resetPosition() {
        this.target.copy(this.target0);
        this.object.parent.position.copy(this.position0);
    }

    /**
     * Function to animate camera flying or target change
     * @param {Vector3} newPosition Position to change
     * @param {Vector3} newTarget New target
     * @param {Number} time Duration of the transition
     * @param {Function} onUpdate Function that receives progress as argument every time, position is changed
     * @param {Function} onComplete Function that is called, when transition is completed
     */
    public flyTo(newPosition: Vector3, newTarget: Vector3, time: number = 1000, onUpdate?: Function, onComplete?: Function) {
        newTarget = newTarget || this.target;
        newPosition = newPosition || this.object.position;
        const oldPos = this.getObjectPosition();
        ...
        //     .onUpdate((delta) => {
        //         const pos = oldPos.clone().lerp(newPosition.clone(), delta);
        //         if (!this.zController)
        //             (this.isVR ? this.object.parent : this.object).position.copy(pos);
        //         else
        //             this.zController.setPosition(pos);
        //         this.target.copy(oldTarget.clone().lerp(newTarget.clone(), delta));
        //         // this.object.lookAt( this.target );
        //         onUpdate && onUpdate(delta);
        //         if (!this.zController) this.update();
        //     })
        //     .onComplete(() => {
    ...
    /**
     * Function to animate camera flying around point change
     * @param {Vector3} circleCenter Point to fly around
     * @param {Number} angle Angle of fly
     * @param {Number} time Duration of the transition
     * @param {Function} onUpdate Function that receives progress as argument every time, position is changed
     * @param {Function} onComplete Function that is called, when transition is completed
     */
    public flyAround(circleCenter: Vector3, angle: number, time: number = 1000, onUpdate?: Function, onComplete?: Function) {
    }
     ...
     */
    private _checkPerspectiveCamera(camera: Object3D): camera is PerspectiveCamera {
     ...
     * @param camera Object to be checked.
     */
    private _checkOrthographicCamera(camera: Object3D): camera is OrthographicCamera {
